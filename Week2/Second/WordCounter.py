...
words= file.lower().replace('?','').replace('.','').replace(',','').split() #make all lowercase and replace , . ? etc to '' SO they can split
...
b=dict(topfive) #convert to dictionaries, to use keys and values on plot bar 
#print(b.values())

# keys=['it','at','or','he','to']
# values=[5,4,4,4,4]

plt.bar(b.keys(),b.values()) #ploting
